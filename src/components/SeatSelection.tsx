...
const SeatSelection = ({ seats, eventId, onSeatSelect, maxSeats = 10 }: SeatSelectionProps) => {
  ...
  const { pricing } = useEventSeatPricing(eventId);
  ...
  const getSeatTotalPrice = (seatCategoryId: string) => {
    const categoryPricing = pricing.find(p => p.seat_category_id === seatCategoryId);
    if (categoryPricing) {
      return categoryPricing.base_price + (categoryPricing.convenience_fee || 0);
    }
    return 0;
  };
  ...
  const handleSeatClick = (seat: Seat) => {
    ...
    setSelectedSeats(prev => {
      ...
      const selectedSeatDetails = newSelection.map(seatId => {
        ...
        const convenienceFee = pricing.find(p => p.seat_category_id === seatData.seat_categories?.id)?.convenience_fee || 0;
