...
import { useEventSeatPricing } from '@/hooks/useEventSeatPricing';
import { createSeatPricingHelpers } from '@/components/seat-selection/seatPricingUtils';
...
const SeatMap = ({ eventId, onSeatSelect }: SeatMapProps) => {
  ...
  const { pricing: eventPricing } = useEventSeatPricing(eventId);
  ...
  // Create pricing helpers
  const { getSeatPrice, getSeatConvenienceFee } = createSeatPricingHelpers(eventPricing);
  ...
  const handleSeatClick = (seatId: string) => {
    ...
    if (onSeatSelect) {
      const selectedSeatObjects = newSelectedSeats.map(id => {
        ...
        const basePrice = getSeatPrice(seatData);
        const convenienceFee = getSeatConvenienceFee(seatData);
                    ...
                    return (
                      <div key={seat.id} className="w-12 h-12 relative">
                        <Button
                          ...
                          title={isBooked ? `${seatDisplayName} - BOOKED - Cannot be selected` : `${seatDisplayName} - ${categoryName} - ₹${basePrice} (+ ₹${convenienceFee} convenience fee at checkout)`}
                          disabled={isBooked}
                        >
                          {isBooked ? (
                            <X className="w-3 h-3" />
